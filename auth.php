 ...
 *
 * @original_author : Bastian Kuhn <bk@mathias-kettner.de>,
 * @author : https://github.com/tomasss1024
 * @changes : changed hash algorithm
 */
...
class auth_plugin_authmultisite extends DokuWiki_Auth_Plugin {
  ...
  function trustExternal($user,$pass,$sticky=false){
      ...
      foreach(array_keys($_COOKIE) AS $cookieName)
      {
        ...
        //if(hash('sha256',$username . $issueTime . $user_secret . $secret) == $cookieHash)
        if(hash_hmac('sha256',$username . $issueTime . $user_secret , $secret) == $cookieHash)
        {
